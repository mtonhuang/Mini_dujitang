...
const topicSubmit = function (e) {
  console.log(this.data)
  let data = {
    text: this.data.text,
    images: this.data.real_files,
    form_id: e.detail.formId
  }
  API.createTopic(data).then(res=>{
    console.log(this.data.real_files)
    if (this.data.text != undefined || this.data.real_files!=undefined ) {
      this.setData({
        success: 0
      })
    } else {
...
const voteSubmit = function (e) {
  ...
  API.vote(topic.id, selectedOption.key).then(res => {
...
//     contxt.setData({ files1: contxt.data.files.concat(res) });
//   }).catch(err => {
